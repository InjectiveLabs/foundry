...
fn add_build_info_constants(output: &mut String, releases: &Releases, platform: svm::Platform) {
    ...
    for build in releases.builds.iter() {
        ...
        let version_key = (version.major, version.minor, version.patch);
        if !seen_versions.insert(version_key) {
            // Avoid duplicate constants when release lists contain repeated versions.
            continue;
        }
...
fn read_releases_from_file(file_path: &PathBuf) -> Releases {
    let file = File::open(file_path)
        .unwrap_or_else(|_| panic!("cannot read releases file at {:?}", file_path));
    serde_json::from_reader(file)
        .unwrap_or_else(|_| panic!("failed to parse JSON from {:?}", file_path))
}

fn load_bundled_releases(platform: svm::Platform) -> Option<Releases> {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let bundled_path = manifest_dir.join(format!("release-list-{platform}.json"));
    if bundled_path.is_file() {
        Some(read_releases_from_file(&bundled_path))
    } else {
        None
    }
}
...
fn generate() {
    ...
    let releases: Releases = if let Ok(file_path) = std::env::var(SVM_RELEASES_LIST_JSON) {
        read_releases_from_file(&PathBuf::from(file_path))
    } else if let Some(releases) = load_bundled_releases(platform) {
        releases
    } else {
